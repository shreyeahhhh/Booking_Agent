...
| 13 | **User changes their mind** | "Actually, let's do the whole flat, not just the sofa" | New items get appended for the rest of the flat; `goods.category` is re-inferred from the fuller item list (`domain.inference.infer_category`, likely landing on `household_mixed`), not corrected directly -- see design.md §3.4 for why category is inferred rather than asked about. Newly-required fields (packing, floors for the new items) appear and are asked. `booking_type` may also be set for summary framing, but it does not gate anything. |
