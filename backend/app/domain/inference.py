"""Guessing the vehicle and helper count from the item list.
...
It never decides anything on its own: the reducer always sets the resulting
field to INFERRED, never CONFIRMED, so the user is always asked to check it.

The scoring is a plain keyword lookup, not a model. That is deliberate: this
guess has to be explainable in one sentence ("a sofa and three cupboards is
about six load units, which maps to a Tata Ace"), and a lookup table is the
simplest thing that gives an honest, defensible answer -- see MASTER_PLAN.md
1.4, "pure function, no AI".
"""
