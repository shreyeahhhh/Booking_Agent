...
def test_locality_renders_cleanly_regardless_of_how_it_diverges_from_evidence():
    """`raw_text` is still recorded (reducer._write_locality_raw_text) and
    still exists for a corrected value to be recoverable from, but the
    summary itself no longer surfaces a "(heard as ...)" caveat -- a
    parenthetical next to every normalised place name read as confusing
    rather than reassuring in practice, per live user feedback. This is a
    presentation choice, not a change to Rule 10's underlying mishearing
    normalisation, which is unaffected."""
    state = _apply(
        BookingState(),
        Patch(
            op=PatchOp.SET,
